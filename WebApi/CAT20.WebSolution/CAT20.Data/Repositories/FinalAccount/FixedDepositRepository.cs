...
namespace CAT20.Data.Repositories.FinalAccount
{
    public class FixedDepositRepository:Repository<FixedDeposit>,IFixedDepositRepository
    {
        ...
        public FixedDepositRepository(DbContext context) : base(context)
        {
        }
        ...
        public async Task<(int totalCount, IEnumerable<FixedDeposit> list)> GetAllFixedDepositForSabha(int sabhaId, Nullable<int> ledgerAccountId, int pageNo, int pageSize, string? filterKeyword)
        {
            var result = voteAccDbContext.FixedDeposit
